...
def run(args):
    ...
    for chunk_idx in tqdm.tqdm(range(num_chunks)):
        ...
        chunk_loader = torch.utils.data.DataLoader(ConcatDataset(buffer), 
        ...
        while not done_with_chunk:
            for inputs, targets in chunk_loader:
                ...
                if optimizer_type == 'sgd':
                    optimizer.step()
                    htcore.mark_step() # This results an error in lazy mode "Internal Error: Received signal - Segmentation fault" if load_to_gpu
        ...
        test_logs, layer_wise_outputs = test(test_loader, model, device)
...
def test(loader, model, device):
    ...
    for inputs, targets in loader:
        ...
        logs = {
            'fisher_trace': fisher_trace.item(),
            'test_loss': loss.item(),
            'test_acc': acc.item()
        }
